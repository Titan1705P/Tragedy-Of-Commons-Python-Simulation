...
class Commoner():
  def __init__(self,up,down,n,model):
    ...
    self.numcows = [n]#list with first entry
    self.model = model
    self.cowslist = []
  def decrease_cows(self):
    remcows = self.numcows[-1]*self.downfactor#to be removed from scheduler
  ...
  def increase_cows(self,o_id):
    addcows = self.numcows[-1]*self.upfactor
...
class Commons(mesa.Model):
  ...
  def __init__(self, width=101,height=99,n_commoners = 10, popl=1000, limit = 100,
    ...
    for i in range(self.initial_population):
      ...
      o_id = i//self.initial_population
      cow = Cow(agent_id, 
                      self,
                      (x,y),
                      moore = False, #nbd
  ...
  def yearstep(self):
    ...
    for i in range(self.n_comm):
      ...
      if(len(self.commoners[i].num_cows)>1):#more than one year, num_cows indictaes the no. of years
          ...
          if(change==0):
            self.commoners[i].inrease_cows(i) 
  ...
  def run_model(self, step_count=25):#run for 25 weeks
...
model = Commons(popl=100,cvision=2)
model.run_model()
